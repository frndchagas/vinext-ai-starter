...
**Last admin**: the only remaining User with the `admin` role. That User cannot be demoted, which preserves an administrative recovery path.
...
**Task**: permanent pedagogical reference for the starter's asynchronous golden path. Owned by a single `User`, it moves through the states `queued`, `processing`, `completed`, or `failed`, and its persisted state is always the source of truth over any realtime notification.
