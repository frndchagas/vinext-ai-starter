...
TypeSpec is the source for application HTTP consumed by the frontend. It emits OpenAPI 3.1, and Orval generates the Fetch client, TanStack Query hooks, Zod schemas and MSW handlers. Laravel keeps native request, resource and domain types. In development and tests, the frontend will validate responses with the generated Zod schemas and fail with endpoint-specific diagnostics; production keeps the generated TypeScript types without response parsing. Oasdiff compares pull requests with their target branch. Feature and browser tests prove implementation behavior; Prism is not a current gate.
